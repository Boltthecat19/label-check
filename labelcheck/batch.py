    ...
    def _run(self, job: BatchJob, apps: list[Application], images: dict[str, bytes], llm) -> None:
        for app in apps:
            ...
            if data is None:
                ...
                job.verdicts.append(
                    Verdict(application_id=app.application_id, warnings=["Image not uploaded."])
                )
            else:
                try:
                    ...
                    job.verdicts.append(
                        Verdict(application_id=app.application_id, warnings=["Could not read the image."])
                    )
            job.done += 1
...
def export_csv(job: BatchJob) -> str:
    ...
    for v in job.verdicts:
        ...
        w.writerow(
            [v.application_id, v.overall.value if v.results else "ERROR"]
            + [
                by[f].status.value if f in by else ""
                for f in ("brand", "abv", "net_contents", "bottler", "origin", "warning")
            ]
