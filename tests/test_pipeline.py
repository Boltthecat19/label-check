...
def _misses(pairs):
    ...
    for m, v in pairs:
        ...
        expected_fields = {
            **{f: Status.PASS for f in by if by[f] != Status.NOT_CHECKED},
            **{k: Status(s) for k, s in m["expect"].items()},
        }
...
def test_augmented_no_false_pass_and_accuracy(verdicts):
    ...
    assert acc >= 0.8, f"augmented accuracy {acc:.2f}\n" + "\n".join(
        f"{f}: {fld} expected {e} got {g}" for f, fld, e, g in misses[:30]
    )
